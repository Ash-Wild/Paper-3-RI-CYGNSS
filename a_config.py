...
cyg_time_from_storm = 30 # minutes. The time from the storm center to consider for matching CYGNSS data with the storm data.
...
l1_variables = ['eff_scatter', 'brcs','sp_inc_angle', 'ddm_snr','gps_eirp', 'rx_to_sp_range', 'sc_vel_x_pvt','sc_vel_y_pvt','sc_vel_z_pvt','sp_theta_orbit','tx_to_sp_range','quality_flags','sv_num', 'ddm_ant','nst_att_status', 'ddm_nbrcs',]
save_parquet_per_storm = False # If True, save the matched data for each storm to a separate parquet file for later analysis. Each storm will have its own parquet file, with the storm_id in the filename.